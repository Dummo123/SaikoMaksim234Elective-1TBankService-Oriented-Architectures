...
class EventHandler:
    ...
    @staticmethod
    def _require_positive_qty(qty, context: str):
        """
        FIX: without this check a negative quantity (e.g. qty=-5 in the DLQ
        demo scenario) would pass the 'available < qty' stock guard
        (0 < -5 is False) and then subtract a negative number, *adding*
        inventory instead of raising.  The event would process "successfully"
        and never reach the DLQ, breaking Scenario 5 of the showcase.
        """
        if qty is None or qty <= 0:
            raise ValueError(
                f"Invalid quantity: {qty} (must be a positive integer) [{context}]"
            )
    ...
    def _product_received(self, event: dict):
        ...
        self._require_positive_qty(qty, f"PRODUCT_RECEIVED product={pid}")
    ...
    def _product_shipped(self, event: dict):
        ...
        self._require_positive_qty(qty, f"PRODUCT_SHIPPED product={pid}")
    ...
    def _product_moved(self, event: dict):
        ...
        self._require_positive_qty(qty, f"PRODUCT_MOVED product={pid}")
    ...
    def _product_reserved(self, event: dict):
        ...
        self._require_positive_qty(qty, f"PRODUCT_RESERVED product={pid}")
    ...
    def _product_released(self, event: dict):
        ...
        self._require_positive_qty(qty, f"PRODUCT_RELEASED product={pid}")
    ...
    def _inventory_counted(self, event: dict):
        ...
        # qty=0 is a valid physical count (empty shelf), so only guard against None/negative
        if qty is None or qty < 0:
            raise ValueError(f"Invalid count quantity: {qty} (must be >= 0) [INVENTORY_COUNTED product={pid}]")
